...
export class MeshHub {
    config;
    storage;
    agents = new Map(); // agentId → ConnectedAgent
    wss;
    purgeTimer = null;
    constructor(config = {}) {
            ...
            ws.on('message', (data) => {
                ...
                switch (msg.type) {
                    ...
                    case 'hub:context:del':
                        ...
                        break;
                    case 'mesh:op': {
                        const result = this.handleMeshOp(msg.payload);
        ...
        try {
            switch (op.op) {
                ...
                case 'read': {
                    ...
                    if (!ctx)
                        return { ok: false, op: 'read', error: `Context ${op.id} not found`, ms: Date.now() - start };
                    return { ok: true, op: 'read', data: ctx, ms: Date.now() - start };
                }
                case 'update': {
                    const existing = this.storage.readContext(op.id);
                    if (!existing)
                        return { ok: false, op: 'update', error: `Context ${op.id} not found`, ms: Date.now() - start };
                    const updated = {
                        ...existing,
                        ...op.patch,
                        id: existing.id,
                ...
                default:
                    return { ok: false, op: op.op, error: 'Unknown op', ms: Date.now() - start };
            }
        }
        catch (err) {
    ...
    findBestAgent(task) {
        const candidates = Array.from(this.agents.values()).filter(a => {
            if (!a.info.capabilities.canDelegate)
                return false;
            if (a.info.status === 'busy')
                return false;
            if (task.targetRuntime && a.info.runtime !== task.targetRuntime)
                return false;
            return true;
        });
    ...
    broadcastToMesh(meshId, msg, excludeAgentId) {
